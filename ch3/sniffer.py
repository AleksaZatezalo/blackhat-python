...
# magic string we'll check ICMP respones for
MESSAGE = "Python rules"

# sprays UDP datagrams with our magic message
def udp_sender():
...
class ICMP:
    """
    A class to represent an ICMP packet.
    """
...
class Scanner:
    def __init__(self, host):
        ...
        if os.name == 'nt':
            self.socket.iotcl(socket.SIO_RCVALL, socket.RCVALL_ON)
    ...
    def sniff(self):
        ...
        try:
            ...
            if os.name == 'nt':
                self.socket.iotcl(socket.SIO_RCVALL, socket.RCVALL_OFF)
            print("\nUser interrupted.")
            if hosts_up:
...
# def sniff(host):
#     """
#     Continual sniffing on address host.
#     """
